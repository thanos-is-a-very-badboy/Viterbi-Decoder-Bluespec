...
{
  init_symbol(&symbols[0u], "inputs", SYM_MODULE, &INST_inputs);
  init_symbol(&symbols[1u], "n_and_m", SYM_MODULE, &INST_n_and_m);
  init_symbol(&symbols[2u], "p_emission", SYM_MODULE, &INST_p_emission);
  init_symbol(&symbols[3u], "p_transition", SYM_MODULE, &INST_p_transition);
  init_symbol(&symbols[4u], "RL_load_m_and_n", SYM_RULE);
  init_symbol(&symbols[5u], "RL_read_66", SYM_RULE);
  init_symbol(&symbols[6u], "RL_read_transition_matrix", SYM_RULE);
  init_symbol(&symbols[7u], "read_transition_tb", SYM_MODULE, &INST_read_transition_tb);
  init_symbol(&symbols[8u], "transition_idx_tb", SYM_MODULE, &INST_transition_idx_tb);
  init_symbol(&symbols[9u], "viterbi", SYM_MODULE, &INST_viterbi);
}
...
void MOD_mkViterbiTestbench::RL_load_m_and_n()
{
  ...
  tUInt32 DEF_outcome__h772;
  tUInt32 DEF_n__h770;
  tUInt32 DEF_m__h771;
  tUInt8 DEF_AVMeth_viterbi_get_n_and_m_loaded;
  DEF_m__h771 = INST_n_and_m.METH_sub(1u);
  DEF_n__h770 = INST_n_and_m.METH_sub(0u);
  DEF_outcome__h772 = INST_inputs.METH_sub(0u);
  ...
  if (DEF_NOT_viterbi_get_n_and_m_loaded___d2)
    INST_viterbi.METH_n_and_m_load(DEF_n__h770, DEF_m__h771, DEF_outcome__h772);
}

void MOD_mkViterbiTestbench::RL_read_66()
{
  tUInt32 DEF_x__h939;
  DEF_x__h939 = INST_viterbi.METH_read_transition_idx();
  INST_read_transition_tb.METH_write((tUInt8)1u);
  INST_transition_idx_tb.METH_write(DEF_x__h939);
}

void MOD_mkViterbiTestbench::RL_read_transition_matrix()
{
  tUInt32 DEF_data__h957;
  tUInt32 DEF_i__h978;
  DEF_i__h978 = INST_transition_idx_tb.METH_read();
  DEF_data__h957 = INST_p_transition.METH_sub(DEF_i__h978);
  INST_viterbi.METH_send_transition_data(DEF_data__h957);
